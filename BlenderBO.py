# -*- coding: utf-8 -*-
"""
Created on Sun Oct 19 18:53:25 2025
...
def rodrigues(pose):
    """
    calculate the the quaternion of the pose matrix
...
class BlenderBO(): 
    def __init__(self, start_frame, end_frame, optimize_epoch, blender_file_name):
        self.start_frame = start_frame
        self.end_frame = end_frame
        self.optimize_epoch = optimize_epoch
        self.blender_file_name = blender_file_name
        return
    
    def command_composer(self):
        """
        based on the script blend/python, compose and return a command
        ...
        cmd = ["blender"]
        return cmd
    def bayeisan_optimize(self):
        
        return
    
    def blender_pose_reset(self):
        return
    
    def blender_mesh_config(self):
        return
...
def membrane_config(start, end, attrib_dict):
    ...
    settings.bending_model = os.environ.get('membrane_mode')
    settings.gravity = Vector((0.0, 0.0, 0.0))
    
    
    settings.quality = 5 # Simulation quality
    settings.mass = 0.000001  # Cloth mass
    settings.air_damping = 1# Air resistance
    print("attributes: ", attrib_dict[0])
    settings.tension_stiffness =attrib_dict[0][0] # Tension stiffness
    settings.compression_stiffness = 0#attrib_dict[0][1] # Shear stiffness
    settings.bending_stiffness = 0#attrib_dict[0][2]  # Bending stiffness
    
    settings.tension_damping = 0#attrib_dict[0][2] # Tension stiffness
    settings.compression_damping = 0#attrib_dict[0][3] # Shear stiffness
    settings.bending_damping = 0  # Bending stiffness
    ...
    #collision_settings.distance_min = 1
...
def load_kinematics(start_frame, end_frame): 
    ...
    armature_name = "Armature.001"  # Change this to match your armature's name
    ...
    for file_index in range(start_frame,end_frame+1): 
            #print(file_index)
        #if file_index == 1:
        json_file_path = f"D:/PhDProject_real_data/brunei_2023_bat_test_13_2/rearrange_pose/{file_index}/membrane_output.json"
...
class Blender_animator(): 
    def __init__(self, start_frame, end_frame, test_name="brunei_2023_bat_test_13_2" , obj_save_root="D:\\PhDProject_real_data\\"): 
