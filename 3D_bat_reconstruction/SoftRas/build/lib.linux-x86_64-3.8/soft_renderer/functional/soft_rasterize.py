...
'''
re-define the softras to make it support non-square images
'''

class SoftRasterizeFunction(Function):

    @staticmethod
    def forward(ctx, face_vertices, textures, image_height=256, image_width = 256,
        ...
        aggrs_info = torch.FloatTensor(ctx.batch_size, 2, ctx.image_height, ctx.image_width).fill_(0.0).to(device=ctx.device) 

        soft_colors = torch.FloatTensor(ctx.batch_size, 4, ctx.image_height, ctx.image_width).fill_(1.0).to(device=ctx.device) 
        ...
        faces_info, aggrs_info, soft_colors = \
            soft_rasterize_cuda.forward_soft_rasterize(face_vertices, textures,
                                                       ...
                                                       soft_colors,
                                                       image_height, image_width, near, far, eps,
...
#        grad_textures = torch.zeros_like(textures, dtype=torch.float32).to(ctx.device).contiguous()
        ...
        return grad_faces, grad_textures, None, None, None, None, None, None, None, None, None, None, None, None, None, None


def soft_rasterize(face_vertices, textures, image_height=256, image_width =256,  
                   background_color=[0, 0, 0], near=1, far=100, 
                   fill_back=True, eps=1e-3,
                   sigma_val=1e-5, dist_func='euclidean', dist_eps=1e-4,
                   gamma_val=1e-4, aggr_func_rgb='softmax', aggr_func_alpha='prod',
    ...
    return SoftRasterizeFunction.apply(face_vertices, textures, image_height, image_width,  
