...
__global__ void forward_soft_rasterize_cuda_kernel(
    ...
    const int bn = i / (ih * iw);
    const int pn = i % (ih * iw);
    ...
    for (int k = 0; k < 3; k++) {
        if (func_id_rgb == 0) { // hard assign, set to background
            soft_color[k] = soft_colors[(bn * 4 + k) * (ih * iw) + pn];
        } else
        if (func_id_rgb == 1) {
            soft_color[k] = soft_colors[(bn * 4 + k) * (ih * iw) + pn] * softmax_sum; // initialize background color
        }
    }
    ...
    if (func_id_alpha == 0) {
        soft_colors[(bn * 4 + 3) * (ih * iw) + pn] =  soft_color[3];
    } else
    if (func_id_alpha == 1) {
        soft_colors[(bn * 4 + 3) * (ih * iw) + pn] =  soft_color[3] / nf;
    } else 
    if (func_id_alpha == 2) {
        soft_colors[(bn * 4 + 3) * (ih * iw) + pn] =  1. - soft_color[3];
    }

    if (func_id_rgb == 0) {
        if (face_index_min != -1)
            for (int k = 0; k < 3; k++) {
                soft_colors[(bn * 4 + k) * (ih * iw) + pn] = soft_color[k];
            }
        aggrs_info[(bn * 2 + 0) * (ih * iw) + pn] = depth_min;
        aggrs_info[(bn * 2 + 1) * (ih * iw) + pn] = face_index_min;
    } else
    if (func_id_rgb == 1) {
        for (int k = 0; k < 3; k++) {
            soft_colors[(bn * 4 + k) * (ih * iw) + pn] = soft_color[k] / softmax_sum;
        }
        aggrs_info[(bn * 2 + 0) * (ih * iw) + pn] = softmax_sum;
        aggrs_info[(bn * 2 + 1) * (ih * iw) + pn] = softmax_max;
    }
}
...
__global__ void backward_soft_rasterize_cuda_kernel(
    ...
    const scalar_t softmax_sum = aggrs_info[(bn * 2 + 0) * (ih * iw) + pn];
    const scalar_t softmax_max = aggrs_info[(bn * 2 + 1) * (ih * iw) + pn];
    ...
    for (int fn = 0; fn < nf; fn++) {
        ...
        // aggragate for alpha channel
        scalar_t C_grad_xy_alpha = grad_soft_colors[(bn * 4 + 3) * (ih * iw) + pn];
        if (func_id_alpha == 0) { // hard assign
        ...
        if (func_id_alpha == 2) { // Logical-Or
            C_grad_xy_alpha *= (1 - soft_colors[(bn * 4 + 3) * (ih * iw) + pn]) / max(1 - soft_fragment, 1e-6);
        }
            ...
            if (fn == softmax_max) {
                for (int k = 0; k < 3; k++) {
                    for (int j = 0; j < texture_size; j++) {
                        atomicAdd(&grad_texture[3 * j + k], backward_sample_texture(grad_soft_colors[(bn * 4 + k) * (ih * iw) + pn], w, texture_res, j, texture_sample_type));
                    }
                }
            }
        } else
        if (func_id_rgb == 1 && (check_face_frontside(face) || double_side)) { // Softmax (Z * D)
            ...
            for (int k = 0; k < 3; k++) {
                const scalar_t grad_soft_color_k = grad_soft_colors[(bn * 4 + k) * (ih * iw) + pn];
                ...
                C_grad_xyz_rgb += grad_soft_color_k * (color_k - soft_colors[(bn * 4 + k) * (ih * iw) + pn]);
            }
...
std::vector<at::Tensor> forward_soft_rasterize_cuda(
    ...
    const dim3 blocks_2 ((batch_size * image_height * image_width - 1) / threads +1);
