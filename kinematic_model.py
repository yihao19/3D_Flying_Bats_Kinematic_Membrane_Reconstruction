...
class Kinematic_model(nn.Module):
    def __init__(self, 
                 ...
                 template_obj_path = "D:/PhD_research/3D_bat_reconstruction/SoftRas/models/membrane_kinematic_optimization_model/dummy.obj",
        ...
        with open(os.path.join("./model_template", bone_skining_matrix_name), 'rb') as f:
            data = pickle.load(f)
    ...
    def forward(self, batch_size, estimated_location, use_previous = False, prev_pose = None):
        ...
        if(use_previous == True):
            ...
            #self.pose_tensor[2][0] = ( prev_pose[2][0] + math.pi / 3 * torch.tanh(self.joint_2[0][0])) 
            ...
            #self.pose_tensor[6][0] =    prev_pose[6][0]  + math.pi / 3 * torch.tanh(self.joint_6[0][0])
            self.pose_tensor[6][1]  =   prev_pose[6][1]  + math.pi / 3 * torch.tanh(self.joint_6[0][1])
            self.pose_tensor[6][2]  =   prev_pose[6][2]  + math.pi / 3 * torch.tanh(self.joint_6[0][2])
            
            #self.pose_tensor[19][0] =   prev_pose[19][0] + math.pi / 3 * torch.tanh(self.joint_19[0][0])
            self.pose_tensor[19][1] =   prev_pose[19][1] + math.pi / 3 * torch.tanh(self.joint_19[0][1])
            self.pose_tensor[19][2] =   prev_pose[19][2] + math.pi / 3 * torch.tanh(self.joint_19[0][2])
           
            
            #self.pose_tensor[7][0] = ( prev_pose[7][0]   + math.pi / 3 * torch.tanh(self.joint_7[0][0]))
            self.pose_tensor[7][1] = ( prev_pose[7][1]   + math.pi / 3 * torch.tanh(self.joint_7[0][1]))
            #self.pose_tensor[7][2] = ( prev_pose[7][2]   + math.pi / 3 * torch.tanh(self.joint_7[0][2]))
            
            #self.pose_tensor[20][0] = ( prev_pose[20][0] + math.pi / 3 * torch.tanh(self.joint_20[0][0]))
            self.pose_tensor[20][1] = ( prev_pose[20][1] + math.pi / 3 * torch.tanh(self.joint_20[0][1]))
            #self.pose_tensor[20][2] = ( prev_pose[20][2] + math.pi / 3 * torch.tanh(self.joint_20[0][2]))
            ...
            #self.pose_tensor[8][0] = ( prev_pose[8][0]   + math.pi / 18 * torch.tanh(self.joint_8[0][0]))
            self.pose_tensor[8][1] = ( prev_pose[8][1]   + math.pi / 18 * torch.tanh(self.joint_8[0][1]))
            ...
            #self.pose_tensor[11][0] = ( prev_pose[11][0] + math.pi / 18 * torch.tanh(self.joint_11[0][0]))
            ...
            #self.pose_tensor[14][0] = ( prev_pose[14][0] + math.pi / 18 * torch.tanh(self.joint_14[0][0]))
            ...
            #self.pose_tensor[21][0] = ( prev_pose[21][0] + math.pi / 18 * torch.tanh(self.joint_21))
            self.pose_tensor[21][1] = ( prev_pose[21][1] + math.pi / 18 * torch.tanh(self.joint_21[0][1]))
            ...
            #self.pose_tensor[24][0] = ( prev_pose[24][0] + math.pi / 18 * torch.tanh(self.joint_24) )
            ...
            #self.pose_tensor[27][0] = ( prev_pose[27][0] + math.pi / 18 * torch.tanh(self.joint_27))
            ...
            if(self.opposite_direction == True):
                ...
                self.pose_tensor[0][1] = math.pi / 2 * torch.tanh(self.joint_0[0][1]) -1 * math.pi# flip the template
                self.pose_tensor[0][2] = math.pi / 2 * torch.tanh(self.joint_0[0][2]) 
            else: 
                self.pose_tensor[0][0] = math.pi / 2 * torch.tanh(self.joint_0[0][0]) #+0.2 * math.pi # for modifying template pose
                self.pose_tensor[0][1] = math.pi / 2 * torch.tanh(self.joint_0[0][1])
                self.pose_tensor[0][2] = math.pi / 2 * torch.tanh(self.joint_0[0][2]) #+0.2 * math.pi
            ...
            self.pose_tensor[2][0] = math.pi / 3 * torch.tanh(self.joint_2[0][0])  # make the neck bone trainable(slightly)
            #for the shoulder using the symmetric deformation
            ...
            #self.pose_tensor[19][0] =  math.pi / 3 * torch.tanh(self.joint_19[0][0])
            self.pose_tensor[19][1] =  math.pi / 3 * torch.tanh(self.joint_19[0][1])
            self.pose_tensor[19][2] =  math.pi / 3 * torch.tanh(self.joint_19[0][2])
            
            
            
            #self.pose_tensor[7][0] = math.pi / 3 * torch.tanh(self.joint_7[0][0])
            self.pose_tensor[7][1] = math.pi / 3 * torch.tanh(self.joint_7[0][1])
            #self.pose_tensor[7][2] = math.pi / 3 * torch.tanh(self.joint_7[0][2])
            
            #self.pose_tensor[20][0] = math.pi / 3 * torch.tanh(self.joint_20[0][0])
            self.pose_tensor[20][1] = math.pi / 3 * torch.tanh(self.joint_20[0][1])
            #self.pose_tensor[20][2] = math.pi / 3 * torch.tanh(self.joint_20[0][2])
            ...
            self.pose_tensor[8][1] = math.pi / 6 * torch.tanh(self.joint_8[0][1])
            #self.pose_tensor[8][:] = math.pi / 6 * torch.tanh(self.joint_8)
            ...
            self.pose_tensor[11][1] = 0.5 * math.pi /  6 * torch.tanh(self.joint_8[0][1])
            #self.pose_tensor[11][:] = math.pi /  6 * torch.tanh(self.joint_11)
            ...
            self.pose_tensor[14][1] = 0.3 * math.pi /  6 * torch.tanh(self.joint_8[0][1])
            #self.pose_tensor[14][:] = math.pi /  6 * torch.tanh(self.joint_14)
            ...
            self.pose_tensor[21][1] = math.pi /  6 * torch.tanh(self.joint_21[0][1])
            #self.pose_tensor[21][:] = math.pi /  6 * torch.tanh(self.joint_21)
            ...
            self.pose_tensor[24][1] = 0.5 * math.pi /  6 * torch.tanh(self.joint_21[0][1])
            #self.pose_tensor[24][:] = math.pi /  6 * torch.tanh(self.joint_24)
            ...
            self.pose_tensor[27][1] = 0.3 * math.pi /  6 * torch.tanh(self.joint_21[0][1])
            #self.pose_tensor[27][:] = math.pi /  6 * torch.tanh(self.joint_27)
